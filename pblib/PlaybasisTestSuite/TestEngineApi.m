...
- (void)testRuleWithCustomFields2 {
    EXP_CREATE(@"rule - custom fields")
    
    BEGIN_AUTHWRAP
    [PBEngineApi rule:[Playbasis sharedPB] playerId:@"mario" action:@"comment" andCompletion:^(PBRule * _Nullable result, NSError * _Nullable error) {
        XCTAssert(error == nil, @"error must be nil");
        EXP_FULFILL
    }];
    END_AUTHWRAP
